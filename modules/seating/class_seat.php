  ...
  function CoordinateToBlockAndName($x, $y, $blockid, $MaxBlockLength = 0, $LinkIt = 0, $userid = 0) {
    ...
    $sepY = 0;
		for ($y = 0; $y <= $block['rows']; $y++) {
			...
			if ($mode != 3) $jscode .= "CreateText('". $this->CoordinateToName(-1, $y, $block['orientation']) ."', ". ($XStartPlan - 10) .", ". ($YOffset + 9) .", '');\n";			
			if ($mode == 1) {
			  if ($sep_cols[$y+1]) $jscode .= "CreateSmallText('^', ". ($XStartPlan - 20) .", ". ($YOffset + 9 + 7) .", 'index.php?mod=seating&action=edit&step=4&blockid=". $_GET['blockid'] ."&change_sep_col=". ($y + 1) ."');\n";
			  else  $jscode .= "CreateSmallText('v', ". ($XStartPlan - 20) .", ". ($YOffset + 9 + 7) .", 'index.php?mod=seating&action=edit&step=4&blockid=". $_GET['blockid'] ."&change_sep_col=". ($y + 1) ."');\n";
      }
      ...
      $sepX = 0;
          ...
          // Show plan				
					default:
						...
						if ($y == 1) $jscode .= "CreateText('". $this->CoordinateToName($x + 1, -1, $block['orientation']) ."', ". ($XOffset - 2) .", ". ($YStartPlan - 6) .", '');\n";
						if ($y == 1 and $mode == 1) {
						  if ($sep_rows[$x+1]) $jscode .= "CreateSmallText('<', ". ($XOffset - 2 + 9) .", ". ($YStartPlan - 16) .", 'index.php?mod=seating&action=edit&step=4&blockid=". $_GET['blockid'] ."&change_sep_row=". ($x + 1) ."');\n";
						  else  $jscode .= "CreateSmallText('>', ". ($XOffset - 2 + 9) .", ". ($YStartPlan - 16) .", 'index.php?mod=seating&action=edit&step=4&blockid=". $_GET['blockid'] ."&change_sep_row=". ($x + 1) ."');\n";
            }
            ...
            $tooltip = addslashes(addslashes($tooltip));
					  ...
					  switch ($seat_state[$y][$x]) {
					    case 0:
					    case 100:
					      if ($mode == 1) $jscode .= "DrawSeatingSymbol(0, $XOffset, $YOffset, '$link', '$tooltip');\n";
					      elseif ($mode == 2) $jscode .= "ClearArea($XOffset, $YOffset, 14, 14, '$link');\n";
					    break;
					    case 2:
              ...
              // No Break!
					    default:
					      if ($mode == 2) $jscode .= "ClearArea($XOffset, $YOffset, 14, 14, '$link');\n";
