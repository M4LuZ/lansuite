...
elseif (!$akt_file) {
	...
	for ($z = 0; $z < $num_pages; $z++) {
        ...
        if ($num_pages > 1){//Multi-page handling
            ...
            if ($_GET['page']<$num_pages-1) $RowContent .=$dsp->FetchLink('[&gt;&gt;Nächste Seite&gt;&gt;]',"index.php?mod=picgallery&file={$_GET["file"]}&page=".($_GET['page']+1)); //add forward button if this is not the last page
